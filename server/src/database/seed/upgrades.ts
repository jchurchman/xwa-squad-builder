...
export function transformUpgradesForDb(rawUpgrades: ImportedUpgrade[]): Omit<Upgrade, "id" | "created_at">[] {
  return rawUpgrades.reduce((transformed, upgrade) => {
    const {
      applies_condition,
      charge,
      chassis,
    ...
    const newUpgrade: Omit<Upgrade, "id" | "created_at"> = {
      ...notNil(applies_condition) && { appliesCondition: Array.isArray(applies_condition) ? applies_condition : [applies_condition] },
      ...notNil(charge) && { charge },
      ...notNil(chassis) && { chassis },
      ...notNil(force) && { force },
      ...notNil(forcerecurring) && { forcerecurring },
      ...notNil(keyword) && { keywords: keyword },
      ...(notNil(unique) || notNil(max_per_squad)) && { maxPerSquad: unique ? 1 : max_per_squad },
      name,
...
function manageRawRestrictions(raw: (string | number)[][]): Partial<Restrictions> {
